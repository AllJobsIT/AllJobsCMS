...
def register_admin_urls():
    urls = [
        path(
            "jsi18n/",
            JavaScriptCatalog.as_view(packages=["parser_ai"]),
...
def register_ai_feature(features):
    ...
    features.register_editor_plugin(
        ...
        "ai",
        ControlFeature(
            js=["js/parser-ai.js"], css={"all": ["css/main.css"]}
        ),
    )
...
def ai_editor_js():
    prompt = Prompt.objects.all().first()
    prompt_json = json.dumps(prompt.as_dict())
    process_url = reverse("parser_ai:process")
